...
import { useAttestations, Attestation } from '../context/AttestationContext';
...
export default function WalletConnectPage() {
  ...
  const [popupAttestation, setPopupAttestation] = useState<Attestation | null>(null);
  ...
  useEffect(() => {
    const fetchWalletinfo = async () => {
      ...
      try {
        ...
        const lovelace = balanceObj.find((asset: Asset) => asset.unit === 'lovelace');
        if (lovelace) {
  ...
  // Simulated function to listen for transactions - replace with real wallet event listener
  useEffect(() => {
    if (!connectedWallet) return;

    // Example: Poll or subscribe to transaction events here
    // For demonstration, simulate a transaction event after 10 seconds
    const timer = setTimeout(() => {
      // Simulated transaction data
      const tx = {
        id: 'tx1234567890',
        type: 'Social Impact Contribution',
        issuer_did: 'did:prism:1234....Soul....Verifier',
        holder_did: 'did:prism:5678....User....Wallet',
        claim_type: 'Social Impact Contribution',
        commitment_period: '2025-Q1',
        schema_id: 'SoulNode-V2.1-Social-Impact',
        timestamp: Math.floor(Date.now() / 1000),
        block_number: 12345678,
        soul_points: 500,
      };

      // Validate transaction using AIKEN code - here we simulate validation success
      const isValid = true; // Replace with actual AIKEN validation call

      if (isValid) {
        const attestation: Attestation = {
          id: tx.id,
          txType: tx.type,
          issuerDid: tx.issuer_did,
          holderDid: tx.holder_did,
          claimType: tx.claim_type,
          commitmentPeriod: tx.commitment_period,
          schemaId: tx.schema_id,
          timestamp: tx.timestamp,
          blockNumber: tx.block_number,
          soulPoints: tx.soul_points,
        };

        // Add attestation to context
        addAttestation(attestation);

        // Show popup
        setPopupAttestation(attestation);
      }
    }, 10000);

    return () => clearTimeout(timer);
  }, [connectedWallet, addAttestation]);
            ...
            <div className="max-w-2xl mx-auto">
              <div className="bg-slate-800 border border-slate-700 rounded-lg p-8 text-center">
                ...
                <p className="text-slate-300 mb-8">
                  To access Soul Node features, you need to connect a Cardano wallet. We support popular wallets like
                  Eternl, Nami, and Flint.
                </p>
